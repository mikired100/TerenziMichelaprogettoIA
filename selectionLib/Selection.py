...
from selectionLib import HeapMin
...
def condOutput(whoami, msg):
    ...
    print(msg)
  #    SAMPLESELECT
def sampleSelect(l, k):
    if (len(l) <= 10) and (len(l)>0):
        insertionSortDown(l)
        return l[k]
    elif len(l) <= 20:
        return trivialSelect(l, k)
    else:
        return quickSelectRand(l, k)
    #     SAMPLEMEDIANSELECT
def sampleMedianSelect(l):
    if printSwitch.dumpOperations:
        print("sampleMedianSelect")
    random.seed(random.randint(0, 100))  # Sceglie un seed random
    m = random.randint(0, 5)  # Sceglie un Intero m random che determinerà la cardinalità del sottoinsieme V
    V = []
    for i in range(0, m):
        el = random.randint(0, m)  # Calcola indice random
        V.append(l[el]) # Inserisci elemento della lista l con indice e in V
    return sampleSelect(V, int(len(V)/2))  #  Ritorna il Mediano di V
