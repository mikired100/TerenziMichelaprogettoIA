...
from selectionLib.HeapMax import HeapMax
...
#QUICKSORT CON SAMPLE MEDIAN SELECT
def quickSortSampleSelect(l):
    recursiveQuickSortSampleSelect(l, 0, len(l) - 1)


def recursiveQuickSortSampleSelect(l, left, right, det=False):
    if printSwitch.dumpOperations:
        print("recursiveQuickSort({},{})".format(left, right))

    if left >= right:
        return
    det = sampleMedianSelect(l)
    mid = partition(l, left, right, det)
    recursiveQuickSortSampleSelect(l, left, mid - 1, det)
    recursiveQuickSortSampleSelect(l, mid + 1, right, det)
...
#QUICKSORT CON SELEZIONE DETERMINISTICA
def quickSortDetSelect(l):
    recursiveQuickSortDetSelect(l, 0, len(l) - 1)


def recursiveQuickSortDetSelect(l, left, right, det=False):
    if printSwitch.dumpOperations:
        print("recursiveQuickSort({},{})".format(left, right))

    if left >= right:
        return
    det = quickSelectDet(l, int(len(l)/2), 10)
    mid = partition(l, left, right, det)
    recursiveQuickSortDetSelect(l, left, mid - 1, det)
    recursiveQuickSortDetSelect(l, mid + 1, right, det)

    if printSwitch.dumpOperations:
        print("- " * left + str(l[left:right + 1]) + " -" * (len(l) - right - 1))

#  QUICK SORT CON SELEZIONE RANDOMICA
def quickSortRandSelect(l):
    recursiveQuickSortRandSelect(l, 0, len(l) - 1)


def recursiveQuickSortRandSelect(l, left, right, det=False):
    if printSwitch.dumpOperations:
        print("recursiveQuickSort({},{})".format(left, right))

    if left >= right:
        return
    det = quickSelectRand(l, int(len(l)/2))
    mid = partition(l, left, right, det)
    recursiveQuickSortRandSelect(l, left, mid - 1, det)
    recursiveQuickSortRandSelect(l, mid + 1, right, det)
...
if __name__== "__main__":
    l2 = [4, 1234, 34, 566, 8, 2, 5346, 8, 3, 263, 7, 8, 3, 7, 57, 2, 43, 87, 845, 42]
    ...
    # heapSort(l)
    #quickSortSampleSelect(l2)  # QUICKSORT WITH SAMPLE MEDIAN SELECTION SORT
    #print(2 << 1)
    print(" ")
